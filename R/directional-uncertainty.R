...
bootstrap_toroidal_ridge <- function(
  ...
  for (b in seq_len(n_boot)) {
    ...
    boot_phi[, b] <- directional_signed_angle(boot$phi)
    boot_rho[, b] <- boot$rho
  }
  ...
  out <- data.frame(
    ...
    phi_lower = apply(boot_phi, 1, stats::quantile, probs = 0.025, na.rm = TRUE),
    phi_upper = apply(boot_phi, 1, stats::quantile, probs = 0.975, na.rm = TRUE),
    rho = base$rho,
...
autoplot.bootstrap_toroidal_ridge <- function(
  ...
  if (quantity == "rho") {
    ...
    ggplot2::ggplot(data, ggplot2::aes(theta_signed, phi_signed)) +
      ggplot2::geom_ribbon(ggplot2::aes(ymin = phi_lower, ymax = phi_upper), fill = ribbon_fill, alpha = 0.35) +
      ggplot2::geom_line(linewidth = 1.05, colour = line_colour) +
      ggplot2::scale_x_continuous(breaks = br$breaks, labels = br$labels) +
