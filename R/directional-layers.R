...
StatToroidalRidge <- ggplot2::ggproto(
  ...
  compute_group = function(
    ...
    out <- data.frame(x = x, y = y, theta = out$theta, phi = out$phi, rho = out$rho)
...
plot_toroidal_topography <- function(
  ...
  ggplot2::ggplot(data, ggplot2::aes(x = theta, y = phi)) +
    ...
    ggplot2::scale_x_continuous(breaks = br$breaks, labels = br$labels, limits = c(-pi, pi)) +
    ggplot2::scale_y_continuous(breaks = br$breaks, labels = br$labels, limits = c(-pi, pi)) +
    ggplot2::scale_fill_gradientn(colours = palette, name = fill_label) +
    ggplot2::coord_equal() +
