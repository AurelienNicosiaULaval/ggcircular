...
#' @param kappa_theta Kernel concentration for `theta`.
#' @param kappa_phi Kernel concentration for `phi`.
#' @param tie_tolerance Relative tolerance used to flag two distinct local
#'   maxima as near-tied. The selected ridge remains the first exact grid
#'   maximum for backward compatibility.
#'
#' @return A data frame with columns `theta`, `phi`, `rho`, `ridge_group`,
...
toroidal_ridge_data <- function(
  ...
  tie_tolerance <- directional_probability_scalar(tie_tolerance, "tie_tolerance")
  ...
  mode_diagnostics <- lapply(seq_len(nrow(est$density)), function(row_id) {
    density <- est$density[row_id, ]
    n_grid <- length(density)
    density_range <- diff(range(density, finite = TRUE))
    flat_tolerance <- sqrt(.Machine$double.eps) * max(1, max(abs(density), na.rm = TRUE))

    if (!is.finite(density_range) || density_range <= flat_tolerance) {
      return(c(
        n_local_modes = n_grid,
        secondary_mode_ratio = 1,
        ridge_ambiguous = 1
      ))
    }

    previous <- density[c(n_grid, seq_len(n_grid - 1L))]
    next_value <- density[c(seq.int(2L, n_grid), 1L)]
    candidates <- which(
      density >= previous & density >= next_value &
        (density > previous | density > next_value)
    )
    ...
    groups <- cumsum(c(TRUE, diff(candidates) > 1L))
    if (length(unique(groups)) > 1L && candidates[1] == 1L && candidates[length(candidates)] == n_grid) {
      groups[groups == max(groups)] <- 1L
    }
    peak_heights <- vapply(
      split(candidates, groups),
      function(index) max(density[index]),
      numeric(1)
    )
    peak_heights <- sort(peak_heights, decreasing = TRUE)
    secondary_ratio <- if (length(peak_heights) >= 2L) {
      peak_heights[2] / peak_heights[1]
    } else {
      NA_real_
    }

    c(
      n_local_modes = length(peak_heights),
      secondary_mode_ratio = secondary_ratio,
      ridge_ambiguous = as.numeric(
        is.finite(secondary_ratio) && secondary_ratio >= 1 - tie_tolerance
      )
    )
  })
  mode_diagnostics <- as.data.frame(do.call(rbind, mode_diagnostics))
  mode_diagnostics$n_local_modes <- as.integer(mode_diagnostics$n_local_modes)
  mode_diagnostics$ridge_ambiguous <- as.logical(mode_diagnostics$ridge_ambiguous)
  ...
  data.frame(
    ...
    n_local_modes = mode_diagnostics$n_local_modes,
    secondary_mode_ratio = mode_diagnostics$secondary_mode_ratio,
    ridge_ambiguous = mode_diagnostics$ridge_ambiguous
  )
}
