...
test_that("conditional density estimates are row-normalized", {
  ...
  topography <- circular_topography_data(cyl$theta, cyl$x, n_theta = 24, n_x = 50)
  expect_named(topography, c("theta", "x", "density"))
  expect_equal(nrow(topography), 24 * 50)
  expect_true(all(is.finite(topography$density)))
  ...
  expect_equal(conditional_variation_index(matrix(1, nrow = 4, ncol = 3)), 0)
  expect_true(conditional_variation_index(est_tor$density) >= 0)
})
